...
exports.csv2Json = function (inFile, outFile, callback) {
	...
	csv()
        ...
        .transform(function (record, index, callback){
	        	...
	        	if (publishers[record.Publisher.name]) {
		        	record.Publisher.url = publishers[record.Publisher.name].url;
	        	} else {
	        		console.log("Warning: found publisher not listed at http://data.gov.uk/publisher: \"" + record.Publisher.name + "\"");
	        	}
	        	// finished */
